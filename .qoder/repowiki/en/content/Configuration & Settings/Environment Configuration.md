...
## Introduction
This document explains how environment configuration is organized and applied across the GameBackend solution. It covers the configuration hierarchy using appsettings.json and environment-specific files such as appsettings.Development.json, environment variable overrides via launch profiles, and practical strategies for managing secrets. It also documents configuration validation, environment detection, and best practices for different deployment environments (development, staging, production).

## Project Structure
Each project in the solution follows a consistent pattern for configuration:
...
- Application code reads configuration using the strongly-typed configuration model and applies it during service registration and middleware setup.
...
**Section sources**
- [GameBackend.API/appsettings.json:1-17](file://GameBackend.API/appsettings.json#L1-L17)
- [GameBackend.API/Program.cs:11-50](file://GameBackend.API/Program.cs#L11-L50)
- [GameBackend.Application/Program.cs:1-44](file://GameBackend.Application/Program.cs#L1-L44)

## Architecture Overview
The configuration architecture relies on the .NET host builder and configuration system. At startup, the host loads configuration from multiple providers, merges them according to precedence, and exposes a unified configuration object to the application. The API project demonstrates:
- Binding JWT settings to a typed model.
- Using the connection string to configure Entity Framework Core.
- Configuring authentication with JWT bearer tokens.
...
Host->>API : Build application
API->>Config : GetSection("Jwt")
API->>API : Bind JwtSettings
API->>DB : UseNpgsql(GetConnectionString("DefaultConnection"))
API->>Auth : Configure JwtBearer with Issuer, Audience, Key
API-->>Host : Ready
```
...
- JWT settings binding: The API project binds the Jwt section to a typed model and configures JWT bearer authentication with issuer, audience, and signing key.
- Database configuration: The API project retrieves the DefaultConnection string and configures Entity Framework Core to use Npgsql.
- Environment-aware middleware: The API project conditionally enables Swagger and SwaggerUI when the environment is Development.

```mermaid
flowchart TD
Start(["Startup"]) --> LoadConfig["Load appsettings.json<br/>and appsettings.Development.json"]
...
- The API project reads the DefaultConnection string and passes it to Entity Framework Core with Npgsql.
...
API->>Config : GetConnectionString("DefaultConnection")
API->>EF : AddDbContext(options => UseNpgsql(...))
...
- JWT authentication is configured using issuer, audience, and a signing key retrieved from configuration.
- Environment detection is performed using the hosting environment; middleware enabling Swagger is only active in Development.

```mermaid
flowchart TD
EnvCheck["Check Hosting Environment"] --> IsDev{"Is Development?"}
...
**Diagram sources**
- [GameBackend.API/Program.cs:59-63](file://GameBackend.API/Program.cs#L59-L63)
- [GameBackend.API/Program.cs:32-50](file://GameBackend.API/Program.cs#L32-L50)
...
## Dependency Analysis
Configuration dependencies across projects:
- The API project consumes configuration for logging, allowed hosts, JWT, and connection strings.
...
```mermaid
graph LR
API["API appsettings.json"] --> JWT["JWT Settings"]
API --> Conn["Connection Strings"]
API --> Log["Logging"]
API --> Hosts["Allowed Hosts"]
...
- JWT validation failures: Confirm issuer, audience, and signing key match the configuration values.
...
## Conclusion
The GameBackend solution employs a clear configuration hierarchy with environment-specific files and launch profiles to manage settings across development, staging, and production. By binding strongly-typed models, using environment variables for overrides, and applying environment-aware middleware, the system supports secure and maintainable deployments. Adopting secret stores and environment-specific configuration files ensures robustness across diverse deployment targets.
